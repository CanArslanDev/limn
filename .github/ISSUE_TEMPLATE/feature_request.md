...
about: Propose a new function, configuration option, error variant, or CLI command for Limn
title: "[feature] "
labels: enhancement
assignees: ''
---
...
```ts
// example: what you want Limn to support
const result = await ai.someNewThing("...", { model: "claude-opus-4-7" });
```

## What Limn does today
...
- [ ] New configuration option (`limn.config.ts` field, env var, per-call option).
...
Limn is organized in three layers (Layer 1 = simple API, Layer 2 = agents + tools, Layer 3 = observability / debug UI). Which layer does this belong in? If it crosses layers, explain how.
