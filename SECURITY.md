...
Limn is a young package on a rolling 0.x release cadence. Until v1.0.0 ships, only the latest minor is actively patched for security issues. After v1.0.0, the policy will widen to the latest two minors.
...
Limn is a thin orchestration layer over Anthropic and OpenAI SDKs. It does not run a server, does not host data, and does not phone home. The trace pipeline writes JSON to a local directory by default (`.limn/traces/`); the optional hosted backend is opt-in and ships nothing without an explicit configuration line.

What Limn explicitly does not do:

- Send anonymous telemetry. The local-first default means zero outbound traffic beyond the LLM provider call you initiated.
- Persist provider API keys to disk. Keys are read from environment variables or `limn.config.ts` and held in memory for the lifetime of the process.
...
- The local inspector (`npx limn inspect`) reads the trace directory; it must never make outbound network calls beyond the LLM provider routes the user already authorized.
...
- The Limn version (`npm ls limn` or `package.json`) and Node.js version (`node --version`).
- Whether the issue is local to Limn itself or involves a specific provider SDK.
...
Limn does not currently operate a bug-bounty program. Credit in release notes and a public thank-you are the only rewards offered.
