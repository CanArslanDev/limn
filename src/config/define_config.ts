/**
 * Identity helper for `traceworks.config.ts`. Lets users get full IntelliSense on
...
import type { RetryConfig, TraceConfig } from "./traceworks_config.js";
 ...
 *
 * Defined explicitly (rather than via `Partial<TraceworksConfig>`) because a
 ...
 */
export interface TraceworksUserConfig {
...
export function defineConfig(config: TraceworksUserConfig): TraceworksUserConfig {
  return config;
}
